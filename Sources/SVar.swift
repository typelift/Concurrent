...
/// An `SVar` ("Sample Variable") is an `MVar` that allows for overwriting its
...
///
/// - Writing to an empty `SVar` fills it with a value and potentially wakes up
///   a blocked reader. (same as for `put()` on an empty `MVar`).
///
